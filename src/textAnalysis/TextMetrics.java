...
public class TextMetrics implements TextAnalysis {
	...
	public Map<String, Integer> wordList() {
		return this.text.getTextDictionary();
	}
	...
	private void findTotals() {
		...
		for (Map.Entry<String, Integer> entry : this.text.getTextDictionary().entrySet()) {

			int numOfSyllables = this.syllables.getSyllableCount(entry.getKey());
			
			// total words
			resultsKeeper[0] += entry.getValue();
			
			// total letters
			resultsKeeper[1] += (entry.getKey().length() * entry.getValue());
			
			// total syllables
			resultsKeeper[2] += (entry.getValue() * numOfSyllables);
			
			// total complex words
			resultsKeeper[3] += numOfSyllables >= 3 ? 1 : 0;
